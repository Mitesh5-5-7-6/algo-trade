...
for (const t of tokens)
  console.log(
    ...
    t.userId,
    "expiresAt:",
    t.expiresAt ? new Date(t.expiresAt).toISOString() : "?",
    "| expired:",
...
for (const k of priceKeys.slice(0, 10))
  console.log("   ", k, "=", await redis.get(k));
const indKeys = await redis.keys("hot:indicators:*");
out("hot:indicators:* keys", indKeys.length);
for (const k of indKeys.slice(0, 5))
  console.log("   ", k, "=", (await redis.get(k))?.slice(0, 160));
