...
for (const s of strategies) {
  const need = NEEDS[s.type]?.(s.params ?? {}) ?? {
    interval: "?",
    bars: NaN,
    why: "unknown type",
  };
  ...
  for (const sym of s.symbols) {
    const row = agg.find(
      (a) => a._id.symbol === sym && a._id.interval === need.interval,
    );
    const have = row?.n ?? 0;
    const ready = have >= need.bars;
    const shortfallMin = ready
      ? 0
      : (need.bars - have) * (need.interval === "1m" ? 1 : 5);
    console.log(
      `      ${sym.padEnd(20)} have ${String(have).padStart(3)}/${need.bars}  ` +
        (ready
          ? "\x1b[32mREADY\x1b[0m"
          : `\x1b[31mNOT READY\x1b[0m — needs ~${shortfallMin} more minutes of feed`),
    );
  }
}
